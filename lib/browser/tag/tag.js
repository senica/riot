...
export default function Tag(impl, conf, innerHTML) {
  ...
  defineProperty(this, 'unmount', function tagUnmount(mustKeepRoot) {
    ...
    var delay = false;
    var e = {
      delay: function(deferred){
        if(!isFunction(deferred)){
          console.warn('e.delay requires one argument. That argument MUST be a function that accepts a resolve parameter. You must call resolve when finished. Going ahead with unmount...');
          return;
        }
        delay = true;
        (new Promise(deferred.bind(this))).then(unmount).catch(unmount);
      }.bind(this)
    }
    this.trigger('before-unmount', e)
